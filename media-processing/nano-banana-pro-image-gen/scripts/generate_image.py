...
# Security: Verify requests version to mitigate PYSEC-2018-28 (SSL cert bypass)
REQUESTS_MIN_VERSION = (2, 22, 0)
_requests_version = tuple(int(x) for x in requests.__version__.split(".")[:3])
if _requests_version < REQUESTS_MIN_VERSION:
    print(
        f"Security Error: requests version {requests.__version__} is vulnerable (PYSEC-2018-28)"
    )
    print(f"Required: requests>={'.'.join(str(x) for x in REQUESTS_MIN_VERSION)}")
    print("Fix: pip install --upgrade 'requests>=2.22.0'")
    sys.exit(1)
    ...
    for content in payload.get("contents", []):
        ...
        for part in content.get("parts", []):
            if (
                ...
                and isinstance(part["inlineData"], dict)
            ):
